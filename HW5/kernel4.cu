...
__global__ void mandelKernel(int *deviceans,float lowerX,float lowerY,float stepX,float stepY,int resX,int resY,int maxIterations)
    ...
    int groupsize= 8;
    int localx,localy;
    localx=(blockIdx.x*blockDim.x + threadIdx.x)*groupsize;
    localy=(blockIdx.y*blockDim.y+threadIdx.y)*groupsize;

    double tmpx,tmpy;
    for(int i=localx;i<localx+groupsize;++i){
      for(int j=localy;j<localy+groupsize;++j){
        if(i>=resX || j >=resY) continue;
        tmpx = lowerX + i*stepX;
        tmpy = lowerY + j*stepY;
        int ans = mandel(tmpx,tmpy,maxIterations);
    
        deviceans[resX* j + i] = ans;
      }
    }
}
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    dim3 numblocks((int)ceil(resX*1.0/(16*8)),(int)ceil(resY*1.0/(16*8)));
